...
def get_university(university_id: int,  db: Session):
    university = db.exec(select(models.University).where(models.University.id == university_id)).first()
    if university == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"This university does not exist")
    return university

@router.get("", response_model= Union[List[schemas.UniversityRes], schemas.UniversityRes])
@auth_check(roles = ["admin"])
def get_get_universities(request: Request, response: Response, id: str = "-1", limit: int = 10, offset: int = 0, order:str="id.asc", q:str="", 
    ...
    order_direction_map = {
        ...
        "conference": [models.University.conference, models.University.conference.desc()],
    ...
    if q != "":
        statement = statement.where(or_(models.University.name.contains(q), 
    ...
    if order_direction == "asc":
        statement = statement.order_by(order_direction_map[order_column][0]).offset(offset).limit(limit)
    else:
        statement = statement.order_by(order_direction_map[order_column][1]).offset(offset).limit(limit)
...
@router.post("", status_code=status.HTTP_201_CREATED, response_model=schemas.UniversityRes)
@auth_check(roles = ["admin"])
def create_university(request: Request, university: schemas.UniversityReq, db: Session = Depends(get_db), Authorize: AuthJWT = Depends(), csrf_protect: CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(request.headers)
    csrf_protect.validate_csrf(csrf_token, request)
    Authorize.jwt_required()
...
@auth_check(roles = ["admin"])
def update_university(request: Request, updated_university: schemas.UniversityReq, id: str = "-1", db: Session = Depends(get_db), Authorize: AuthJWT = Depends(), csrf_protect: CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(request.headers)
    csrf_protect.validate_csrf(csrf_token, request)
    Authorize.jwt_required()
    if id != "-1":
...
@auth_check(roles = ["admin"])
def delete_university(request: Request, id: str = "-1", db: Session = Depends(get_db), Authorize: AuthJWT = Depends(), csrf_protect: CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(request.headers)
    csrf_protect.validate_csrf(csrf_token, request)
    Authorize.jwt_required()
    ...
    for university_id in university_ids:
        university = db.exec(select(models.University).where(models.University.id == university_id)).first()
        if university == None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"University with id: {university_id} does not exist")
